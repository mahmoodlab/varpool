...
parser.add_argument('--arch', type=str, default='amil_slim',
                    choices=['amil_slim', 'amil_nn', 'amil_var_nn', 'sum_mil', 'sum_var_mil', 'patchGCN', 'patchGCN_varpool', 'amil_gcn', 'amil_gcn_varpool'],
...
if args.mode == 'patch':
    ...
    ds_train = GraphDataset(train_fpaths, y_train, task, args.arch)
    dataset_train = ds_train()
    ds_val = GraphDataset(val_fpaths, y_val, task, args.arch)
    dataset_val = ds_val()
...
if task == 'clf':
    ...
    if args.imbal_how == 'resample':
        ...
        # To accommodate for GraphDatasets
        sample_weights = \
            get_weights_for_balanced_binXcensor(surv_df=y_train,
                                                time_bin_col='time_bin',
                                                censor_col='censorship')
...
optimizer = get_optim(model=model, algo=args.opt,
                      lr=args.lr, weight_decay=args.weight_decay)


# TODO: add cyclic and cosine
if args.lr_scheduler == 'plateau':
    scheduler = ReduceLROnPlateau(optimizer=optimizer,
                                  # factor=0.1, patience=10,
                                  factor=args.plateau_factor,  # np.sqrt(.1),
...
################
print('\n\nStart training')
desc_prefix = '{}, '.format(args.name) if args.name is not None else ''
for epoch in tqdm(range(args.n_epochs), desc=desc_prefix + 'epoch progress'):

    # possilby unfree var pool
    if args.freeze_var_epochs is not None\
            and (epoch + 1) > args.freeze_var_epochs\
            and hasattr(model, 'var_pool_on'):
        model.var_pool_on()

    # tqdm_desc = desc_prefix + 'training, epoch {}'.format(epoch)
    tqdm_desc = None
    ...
    if overwrite_loss_metric_key is not None:
        train_loss = train_metrics[overwrite_loss_metric_key]

    # track the loss and other evaulation metrics
    if writer:
        ...
        if evaler is not None:
            for (k, v) in train_metrics.items():
                writer.add_scalar(tag='train/{}'.format(k),
                                  scalar_value=v,
                                  global_step=epoch)

    # tqdm_desc = desc_prefix + 'validation, epoch {}'.format(epoch)
    tqdm_desc = None
    ...
    # the loss properly so we obtain it from the Evaler object
    if overwrite_loss_metric_key is not None:
        val_loss = val_metrics[overwrite_loss_metric_key]

    print('epoch {} train loss = {}, val loss = {}'.
          format(epoch, train_loss, val_loss))
    print('epoch {} train metrics:'.format(epoch), train_metrics)
    print('epoch {} val metrics:'.format(epoch), val_metrics)
    ...
    if writer:
        ...
        if evaler is not None:
            for (k, v) in val_metrics.items():
                writer.add_scalar(tag='val/{}'.format(k),
                                  scalar_value=v,
                                  global_step=epoch)
    ...
    if args.early_stopping:
        ...
        # do early stopping on validation set score
        if args.es_monitor == 'metric':
            # score a metric e.g. c-index, auc
            score = val_metrics[early_stop_metric]
        elif args.es_monitor == 'loss':
            # score is the val loss
            score = val_loss

        stop_early = early_stopper(model=model,
                                   score=score,
                                   epoch=epoch,
                                   ckpt_name='s_checkpoint.pt')

        if stop_early:
            print("{}stopping early after completing {} epochs".
                  format(desc_prefix, epoch + 1))
            break

    # possibly update learning rate scheduler
...
if args.early_stopping:
    ...
    # this is the best model so save it
    torch.save(model.state_dict(), checkpoint_fpath)


runtime = time() - start_time
print("\n\nFinished training after {:1.2f} minutes\n\n".
      format(runtime/60))
...
epochs_with_records = early_stopper.epochs_with_records_ \
    if early_stopper is not None else None

if args.mode == 'patch':
    # Write text summary of results
    summary = write_training_summary(fpath=summary_fpath,
                                     task=task, y_split=y_split, args=args,
                                     runtime=runtime,
                                     ...
                                     model=model,
                                     loss_func=loss_func,
                                     train_bag_size_summary=train_bag_size_summary,
                                     n_bag_feats=n_bag_feats,
                                     n_epoch_completed=epoch + 1,
                                     epochs_with_records=epochs_with_records)

    print(summary)

# save numerical results
results = {**{'val_' + k: v for (k, v) in final_val_metrics.items()},
           'val_loss': final_val_loss,
           **{'train_' + k: v for (k, v) in final_train_metrics.items()},
           'train_loss': final_train_loss,
           'runtime': runtime,
           'n_epoch_completed': epoch + 1,
           }

# the epoch we got the model weights from
if early_stopper is not None:
    results['record_epoch'] = max(early_stopper.epochs_with_records_) + 1
